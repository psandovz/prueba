...
export class DatosComponent implements OnInit {
  ...
  constructor() {
    this.forma = new FormGroup({
      nombreCompleto: new FormGroup({
        ...
        apellido: new FormControl('', [Validators.required, Validators.minLength(8), this.noSandoval] )
      }),
      cargo: new FormControl('', [Validators.required, this.noAdministrador]),
  ...
  noSandoval(control: FormControl): {[s: string]: boolean} {
    if (control.value === 'Sandoval') {
      return { noSandoval: true };
    }
  }

  noAdministrador(control: FormControl): {[s: string]: boolean} {
